...
\resumeItemListStart
\resumeItem{Straggler Mitigation for Cloud Databases}
{Proposed an improved computing layer to mitigate the straggler problem for
  cloud databases with limited computing resources through task replication.
  Developed a cache management system to duplicate and manage hot caches for
  replicated tasks in order to further reduce the latency caused by reading the
  remote storage layer. Improved the consistent hashing based scheduling
  mechanism to better utilize multiple copies of caches while balancing skewed
  workloads.}
\resumeItemListEnd
